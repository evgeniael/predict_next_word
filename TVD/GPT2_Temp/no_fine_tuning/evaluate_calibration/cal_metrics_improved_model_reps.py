...
def get_provo_data():
    ...
    #os.chdir(os.path.join(os.getcwd(), 'output')) #for working node
    os.chdir(os.getcwd()) #when submitting job
...
class metrics:
    ...
    def get_metrics_data_for_disjoint_oracles(self, seed_oracle, N):
        ...
        for key in self.dataset.keys():
            ...
            oracle_obj = oracle(data_per_context, seed_oracle)
            ...
            human_1, human_2 = oracle_obj.sample_oracle_without_replacement_disjoint_groups(N)
    ...
    def get_metrics_data_for_bootstrapped_oracles(self, seed_oracle, N):
        dict_out = {'tvd_val': {'tvd_per_instance_biased_model_human1' : [],
                                'tvd_per_instance_unbiased_model_human1' : [],
                                'tvd_per_instance_human2_human1' : []}, 
                    'ent_diff_val': {'ent_diff_per_instance_biased_model_human1' : [],
        ...
        for key in self.dataset.keys():
            ...
            #Obtaining distributions for all oracle estimators (list of words and a list of their respective probs)
            human_1, human_2 = oracle_obj.sample_oracle_with_replacement_bootstrapping(N)
    ...
    def get_metrics_data(self):
        ...
        for seed_oracle in range(self.reps):
            dict_final[seed_oracle] = {'disjoint_oracles': {}, 'bootstrapped_oracles': {}}

            dict_final[seed_oracle]['disjoint_oracles'][10] = self.get_metrics_data_for_disjoint_oracles(seed_oracle, 10)
            dict_final[seed_oracle]['disjoint_oracles'][20] = self.get_metrics_data_for_disjoint_oracles(seed_oracle, 20)

            dict_final[seed_oracle]['bootstrapped_oracles'][10] = self.get_metrics_data_for_bootstrapped_oracles(seed_oracle,10)
            dict_final[seed_oracle]['bootstrapped_oracles'][20] = self.get_metrics_data_for_bootstrapped_oracles(seed_oracle,20)
            dict_final[seed_oracle]['bootstrapped_oracles'][30] = self.get_metrics_data_for_bootstrapped_oracles(seed_oracle,30)
            dict_final[seed_oracle]['bootstrapped_oracles'][40] = self.get_metrics_data_for_bootstrapped_oracles(seed_oracle,40)
    ...
    def calculate_metrics(self):
        ...
        for oracle_seed in dict_out.keys():
            dict_results[oracle_seed] = {}
            for type_oracle in dict_out[oracle_seed].keys():
                dict_results[oracle_seed][type_oracle] = {}
                for oracle_size in dict_out[oracle_seed][type_oracle].keys():
                    dict_results[oracle_seed][type_oracle][oracle_size] = {}
                    for n in self.n_list: #proportions of improvement
                        dict_results[oracle_seed][type_oracle][oracle_size][n] = {}
                        
                        #Setting a random seed for each value of n 
                        np.random.seed(int(n*10))
            
                        #TVD
                        tvd_oracle_eval = dict_out[oracle_seed][type_oracle][oracle_size]['tvd_val']['tvd_per_instance_human2_human1']

                        tvd_model_eval_biased = dict_out[oracle_seed][type_oracle][oracle_size]['tvd_val']['tvd_per_instance_biased_model_human1']
            
                        tvd_mixed_eval_biased = np.where(np.random.random((len(tvd_oracle_eval))) > n, tvd_model_eval_biased, tvd_oracle_eval)
                        dict_results[oracle_seed][type_oracle][oracle_size][n]['tvd_biased'] = list(tvd_mixed_eval_biased)
            
                        tvd_model_eval_unbiased = dict_out[oracle_seed][type_oracle][oracle_size]['tvd_val']['tvd_per_instance_unbiased_model_human1']
            
                        tvd_mixed_eval_unbiased = np.where(np.random.random((len(tvd_oracle_eval))) > n, tvd_model_eval_unbiased, tvd_oracle_eval)
                        dict_results[oracle_seed][type_oracle][oracle_size][n]['tvd_unbiased'] = list(tvd_mixed_eval_unbiased)

                        #Entropy difference

                        ent_diff_oracle_eval = dict_out[oracle_seed][type_oracle][oracle_size]['ent_diff_val']['ent_diff_per_instance_human2_human1']

                        ent_diff_model_eval_biased = dict_out[oracle_seed][type_oracle][oracle_size]['ent_diff_val']['ent_diff_per_instance_biased_model_human1']
                        ...
                        dict_results[oracle_seed][type_oracle][oracle_size][n]['ent_diff_biased'] = list(ent_diff_mixed_eval_biased)
            
                        ent_diff_model_eval_unbiased = dict_out[oracle_seed][type_oracle][oracle_size]['ent_diff_val']['ent_diff_per_instance_unbiased_model_human1']
                        ...
                        dict_results[oracle_seed][type_oracle][oracle_size][n]['ent_diff_unbiased'] = list(ent_diff_mixed_eval_unbiased)

        return dict_results

d = get_provo_data()

#data = dict(list(d.items())[:3])

metrics_obj = metrics(dataset = d)   

dict_results = metrics_obj.calculate_metrics()

with open("cal_metrics_improved_model_reps.json", "w") as outfile:
