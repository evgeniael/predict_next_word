...
from transformers import GPT2Tokenizer, GPT2LMHeadModel, BloomTokenizerFast, BloomForCausalLM
...
class ancestral:
    def __init__(self, model, tokenizer, temp):
        self.model = model
        self.tokenizer = tokenizer
        ...
        self.seed = 0
        self.add_tokens = 5
        self.pad_token = len(tokenizer) - 1
        self.top_k = 0
    ...
    def get_ancestral_words(self, context):
        ...
        outputs = self.model.generate(inputs_ids, do_sample=True, num_beams = 1, num_return_sequences= self.n_samples, 
                                      ...
                                      length_penalty = self.length_penalty, temperature= self.temperature)
...
def construct_dataset(paragraphs_provo, tokenizer, model, lower_bound, upper_bound, temp):
    ...
    for text_id in range(lower_bound, upper_bound): #iterate over all provo paragraphs
        ...
        for word_num in paragraphs_provo[text_id]['predictions'].keys():
            ...
            #ANCESTRAL
            ancestral_obj = ancestral(model, tokenizer, temp)
...
def initiate_dataset_construction(input_data, pre_trained_model = "gpt2", lower_bound = 1, upper_bound = 2, temp = 0.5, model_directory = None):
    ...
    elif pre_trained_model == "gpt2_fine_tuned_multi_label":
        tokenizer = GPT2Tokenizer.from_pretrained(model_directory)
        model = GPT2LMHeadModel.from_pretrained(model_directory)
    elif pre_trained_model == "bloom":
        tokenizer = BloomTokenizerFast.from_pretrained(model_directory)
        model = BloomForCausalLM.from_pretrained(model_directory)

    dict_dataset = construct_dataset(paragraphs_provo, tokenizer, model, lower_bound, upper_bound, temp)

    #file_name = os.path.join(output_folder, f"Paragraphs-{lower_bound}-{upper_bound - 1}.json")
    file_name = f"Paragraphs_temp-{lower_bound}-{upper_bound - 1}.json"
...
initiate_dataset_construction(input_data, pre_trained_model = "gpt2", lower_bound = 43, upper_bound = 56, temp = 0.5, model_directory = model_dir)